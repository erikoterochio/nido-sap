...
async function main() {
  ...
  const departamentos = await Promise.all([
    prisma.departamento.create({ data: { nombre: 'Defensa I', direccion: 'Defensa 1035', duenos: ['Facundo Vega'], comisionPorcentaje: 0.20, quienPagaServicios: 'BENVEO', descuentoBajarDinero: true } }),
    prisma.departamento.create({ data: { nombre: 'Defensa II', direccion: 'Defensa 912', duenos: ['Facundo Vega'], comisionPorcentaje: 0.20, quienPagaServicios: 'BENVEO', descuentoBajarDinero: true } }),
    prisma.departamento.create({ data: { nombre: 'Eslovenia I (502)', direccion: 'Eslovenia 502', duenos: ['Bautista Leivar'], comisionPorcentaje: 0.20, quienPagaServicios: 'BENVEO' } }),
    prisma.departamento.create({ data: { nombre: 'Eslovenia II (904)', direccion: 'Eslovenia 904', duenos: ['Bautista Leivar'], comisionPorcentaje: 0.20, quienPagaServicios: 'BENVEO' } }),
    prisma.departamento.create({ data: { nombre: 'Araoz', direccion: 'Araoz 2288', duenos: ['Facundo Vega', 'Bautista Leivar'], comisionPorcentaje: 0.20, quienPagaServicios: 'BENVEO' } }),
    prisma.departamento.create({ data: { nombre: 'Moreno', direccion: 'Moreno 550', duenos: ['Eduardo Fontenla'], comisionPorcentaje: 0.20, quienPagaServicios: 'DUENO' } }),
    prisma.departamento.create({ data: { nombre: 'Petit', direccion: 'Petit', duenos: ['Owner'], comisionPorcentaje: 0.20, quienPagaServicios: 'BENVEO' } }),
    prisma.departamento.create({ data: { nombre: 'Azopardo', direccion: 'Azopardo 770', duenos: ['Bautista Leivar'], comisionPorcentaje: 0, quienPagaServicios: 'DUENO' } }),
  ])
  ...
  const mesActual = new Date(2025, 0, 1)
  ...
  const turnosEjemplo = [
    ...
    { depto: departamentos[6], empleado: sandra,  fecha: new Date(2025, 0, 11), entrada: '11:00', salida: '23:00' },
  ...
  for (const t of turnosEjemplo) {
    ...
    await prisma.turnoLimpieza.create({
      data: {
        ...
        montoTotal:     monto,
        mes:            mesActual,
        estado:         alertas.length > 0 ? 'PENDIENTE_REVISION' : 'APROBADO',
        alertas,
      },
    })
  }
